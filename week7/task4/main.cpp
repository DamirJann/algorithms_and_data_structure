...
private:
    ...
    bool isList(const int& currentNode);
    int getMax(const int& currentNode);
public:
...
int Tree::remove(const int& key){
    ...
    if (size == 0) {
        ...
        if (isList(currentNode)){
            startNode = getParent(currentNode);
            if (currentNode != root) {
        ...
        else if (getLeft(currentNode) == 0){
            startNode = getRight(currentNode);
            if (currentNode != root) {
                if (getLeft(getParent(currentNode)) == currentNode) {
                    array[getParent(currentNode)].left = getRight(currentNode);
                } else {
                    array[getParent(currentNode)].right = getRight(currentNode);
                }
            }
            else{
                root = getRight(currentNode);
            }
            array[getRight(currentNode)].parent = getParent(currentNode);
        }
        else{
            int maxNode = getMax(getLeft(currentNode));

            if (getParent(maxNode) == currentNode){
                startNode = maxNode;
                array[maxNode].right = getRight(currentNode);
                array[maxNode].parent = getParent(currentNode);
                if (getParent(currentNode) != 0){
                    if (getLeft(getParent(currentNode)) == currentNode){
                        array[getParent(currentNode)].left = maxNode;
                    }
                    else{
                        array[getParent(currentNode)].right = maxNode;
                    }
                }
            }
            else{
                startNode = getParent(maxNode);



                array[getParent(maxNode)].right = getLeft(maxNode);

                array[maxNode].left = getLeft(currentNode);
                array[maxNode].right = getRight(currentNode);

                array[getLeft(maxNode)].parent = maxNode;
                array[getRight(maxNode)].parent = maxNode;
...
int Tree::getNode(const int& key) const{
    if (size == 0) {
        ...
        while (getValue(currentNode) != key){
...
bool Tree::isList(const int& currentNode){
    return (getLeft(currentNode) == 0 && getRight(currentNode) == 0);
}

int Tree::getMax(const int& currentNode){
    int maxNode = currentNode;
    while (getRight(maxNode) != 0){
        maxNode = getRight(maxNode);
    }
    return maxNode;
}